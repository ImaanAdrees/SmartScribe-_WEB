...
function SystemMaintenance() {
  ...
  const fetchSystemInfo = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/maintenance/system-info`,
        {
          headers: {
  ...
  const fetchUpdateHistory = async () => {
    try {
      const token = localStorage.getItem("adminToken");
      const response = await axios.get(
        `${API_URL}/api/maintenance/update-history`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchBackupHistory = async () => {
    try {
      const token = localStorage.getItem("adminToken");
      const response = await axios.get(
        `${API_URL}/api/maintenance/backup-history`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchBackupConfig = async () => {
    try {
      const token = localStorage.getItem("adminToken");
      const response = await axios.get(
        `${API_URL}/api/maintenance/backup-config`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (data.success) {
        ...
        setBackupConfigState({
          ...
          oneTimeScheduledBackup: data.data.backupConfig.oneTimeScheduledBackup
            ? new Date(data.data.backupConfig.oneTimeScheduledBackup)
                .toISOString()
                .slice(0, 16)
            : "",
        });
  ...
  const handleMaintenanceModeToggle = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/maintenance/toggle-mode`,
  ...
  const handleAPKUpload = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem("adminToken");
      const response = await axios.post(
        `${API_URL}/api/maintenance/upload-apk`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleAutoBackupToggle = async () => {
    try {
      const token = localStorage.getItem("adminToken");
      const response = await axios.post(
        `${API_URL}/api/maintenance/update-backup-config`,
  ...
  const handleTriggerBackup = async () => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const response = await axios.post(
        `${API_URL}/api/maintenance/trigger-backup`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteVersion = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${API_URL}/api/maintenance/delete-apk/${apkToDelete}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const scheduledTasks = [
    {
      ...
      lastRun: "2024-10-27 02:00",
      status: "Completed",
    },
    {
      task: "Log Cleanup",
      schedule: "Weekly on Sunday",
      lastRun: "2024-10-20 03:00",
      status: "Completed",
    },
    {
      task: "Cache Clear",
      schedule: "Every 6 hours",
      lastRun: "2024-10-27 12:00",
      status: "Completed",
    },
    {
      task: "Security Scan",
      schedule: "Daily at 4:00 AM",
      lastRun: "2024-10-27 04:00",
      status: "Completed",
    },
  ];
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">
                      Deploy the latest version to users
                    </p>
                  </div>
                </div>
                <button
                  ...
                  <div className="group">
                    ...
                    <div className="relative border-2 border-dashed border-gray-200 group-hover:border-indigo-400 rounded-2xl p-8 transition-all bg-gray-50/50 hover:bg-indigo-50/30 text-center">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-gray-500 text-sm">
                          Support .apk and .ipa files up to 100MB
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-6">
          ...
          <div className="p-4 bg-gray-50/50 rounded-2xl border border-gray-100 flex justify-between items-center transition-all hover:bg-gray-50">
            ...
            >
              <span
                className={`inline-block h-5 w-5 transform bg-white rounded-full transition-transform duration-300 shadow-sm
          ...
          <form onSubmit={handleBackupConfigUpdate} className="space-y-6">
            ...
            <div className="p-4 bg-indigo-50/30 rounded-2xl border border-indigo-100/50 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Enable automated daily system backups
                </p>
              </div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <input
                    ...
                    onChange={(e) =>
                      setBackupConfigState({
                        ...backupConfig,
                        oneTimeBackupEnabled: e.target.checked,
                      })
                    }
                    className="w-4 h-4 text-indigo-600 rounded focus:ring-indigo-500"
                  />
                  <label
                    htmlFor="oneTimeBackup"
                  ...
                  <div className="animate-fadeIn">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setBackupConfigState({
                          ...backupConfig,
                          oneTimeScheduledBackup: e.target.value,
                        })
                      }
            ...
            <div>
              ...
              <p className="font-semibold">
                {systemInfo.nextScheduledBackup
                  ? new Date(systemInfo.nextScheduledBackup).toLocaleString(
                      "en-US",
                      {
                        month: "short",
                        day: "numeric",
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
        ...
        {recentUpdates.length === 0 ? (
          ...
          recentUpdates.map((update, idx) => (
            <div
              key={idx}
              className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 bg-gray-50 rounded-xl"
            >
              <div
          ...
          <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-fade-in text-center border border-gray-100">
              ...
              <p className="text-gray-600 mb-8 leading-relaxed">
                Are you sure you want to delete this version? This action will
                permanently remove the file from the server and cannot be
                undone.
              </p>
